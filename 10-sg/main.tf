module "frontend" {
    ...
    source = "git::https://github.com/Shirisha-Kodari/terraform-aws-securitygroup.git?ref=main"
...
resource "aws_security_group_rule" "bastion_laptop" {
  type              = "ingress"
  from_port         = 22 #enable to ports for logging to bastion 
  to_port           = 22
  protocol          = "tcp"
...
resource "aws_security_group_rule" "backend_alb_vpn" {
  ...
  from_port         = 80
  to_port           = 80
  protocol          = "tcp" #if allows specific port
  source_security_group_id = module.vpn.sg_id #source = vpn sg_id ,traffic allows from vpn
  security_group_id = module.backend_alb.sg_id #this sg for alb 
}
#backend alb accepting connections from my vpn host on port no 80
...
resource "aws_security_group_rule" "redis_user" { #This Terraform code allows incoming TCP traffic
                                             # on port 6379 (Redis) from the User application security group to the Redis             
  type              = "ingress"
  from_port         = 6379
  to_port           = 6379
  protocol          = "tcp"
...
#This Terraform rule allows MySQL access (TCP 3306) from the Shipping application security group to the MySQL security group.
resource "aws_security_group_rule" "mysql_shipping" {
  type              = "ingress"
  from_port         = 3306
  to_port           = 3306
  protocol          = "tcp"
  source_security_group_id = module.shipping.sg_id #shipping sg 
  security_group_id = module.mysql.sg_id #add mysql sg to shipping sg open port 3306 access traffic from shipping
}
...
resource "aws_security_group_rule" "cart_bastion_ssh" { #connections from bastion to cart 
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
...
resource "aws_security_group_rule" "cart_vpn_http" { #connections from vpn to cart 
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
  protocol          = "tcp"
...
resource "aws_security_group_rule" "payment_vpn_httpS" {
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
  protocol          = "tcp"
...
resource "aws_security_group_rule" "sonarqube_laptop" {
  type              = "ingress"
  from_port         = 0 #enable to ports for logging to bastion 
  to_port           = 0
  protocol          = "-1"
  cidr_blocks       = ["0.0.0.0/0"] 
  security_group_id = module.sonarqube.sg_id
}